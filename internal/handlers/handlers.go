...
func (m *Repository) Reservation(w http.ResponseWriter, r *http.Request) {
	// pull the reservation out of session
	...
	// extract RoomName by querying DB using res.RoomID
	...
	// Update the session context to have the RoomName
	m.App.Session.Put(r.Context(), "reservation", res)

	// type cast back start-date & end-date from time format to string to being displayed for user Form
...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	// send notification - first to the guest
	htmlMessage := fmt.Sprintf(`
		<strong>Reservation Confirmation </strong><br>
		Dear %s: <br>
		This is confirm your resrvation from %s to %s.`,
...
func (m *Repository) PostAvailability(w http.ResponseWriter, r *http.Request) {
	...
	// convert start-date end-date from string to time format
	layout := "2006-01-02"
...
func (m *Repository) ChooseRoom(w http.ResponseWriter, r *http.Request) {
	// 1. get the room id
	// roomID hold the extracted param which is id from the url request
	roomID, err := strconv.Atoi(chi.URLParam(r, "id"))
	if err != nil {
		helpers.ServerError(w, err)
		return
	}
	// 2. get the reservation variable that stored in the session
	// pull the reservation out of session
	res, ok := m.App.Session.Get(r.Context(), "reservation").(models.Reservation)
	if !ok {
		helpers.ServerError(w, err)
	...
	// update RoomID to the extracted id
	res.RoomID = roomID

	// put the updated reservation info(RoomID) back to the session
	m.App.Session.Put(r.Context(), "reservation", res)

	http.Redirect(w, r, "/make-reservation", http.StatusSeeOther)
}

// BookRoom takes URL parameters, builds a session variables, and direct user to make-reservation page
func (m *Repository) BookRoom(w http.ResponseWriter, r *http.Request) {
	// get from session id (s)start (e)end
	roomID, _ := strconv.Atoi(r.URL.Query().Get("id"))
	sd := r.URL.Query().Get("s")
	ed := r.URL.Query().Get("d")
...
func (m *Repository) PostShowLogin(w http.ResponseWriter, r *http.Request) {
	...
	form.IsEmail("email")
	if !form.Valid() {
		// take user back to login form page
		render.Template(w, r, "login.page.tmpl", &models.TemplateData{
			Form: form,
		})
		return
...
// AdminNewReservations show all new reservations for admin
...
func (m *Repository) AdminReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	// 1 assume there is no month/year specified
	now := time.Now()

	// 2 if there is key: "y" then assign the year&month to current time
	...
	for _, x := range rooms {
		...
		// get all the restrictions for the current room in current month
		restrictions, err := m.DB.GetRestrictionsForRoomByDate(x.ID, firstOfMonth, lastOfMonth)
		if err != nil {
			helpers.ServerError(w, err)
		}
		// determine whether restriction reserved or blocked
...
func (m *Repository) AdminShowReservation(w http.ResponseWriter, r *http.Request) {
	// get the url then extract the page whether {all} or {new}, then the id
